    ...
    for iobj in range(len(handle_tickers)):
        for ibin in range(len(handle_tickers[iobj])):
            ...
            if cont:
                ...
                image_ax.imshow(solutions[ibin].counterfactualImage)
            else:
...
def plot_front(front, front_ax):
    ...
    front_ax.set_xlabel('Code D')
    front_ax.set_ylabel('Prediction')
    front_ax.set_zlabel('Plausibility')
    front_ax.scatter3D(x_data, y_data, z_data, cmap='Greens')


def chord_diagram(solutions: List[FloatSolution], nbins='auto', ax=None,
                  fig=None, obj_labels=None,
                  prop_labels=dict(fontsize=13, ha='center', va='center'),
                  pad=6, image_ax=None, front_ax=None):

    if front_ax is not None:
        plot_front(solutions, front_ax)
    ...
    fig.canvas.mpl_connect("motion_notify_event",
                           lambda event: hover_over_bin(event,
                                                        handle_tickers,
                                                        handle_plots,
                                                        colors,
                                                        fig,
