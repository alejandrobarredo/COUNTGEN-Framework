...
for i, data in enumerate(islice(dataset, opt.num_test)):
    ...
    for nn in range(opt.n_samples + 1):
        ...
        with open('./imgs/edges2shoes/test_images-' + str(nn) + '.pkl', \
                'wb') as f:
            pkl.dump([real_A, fake_B, real_B], f, pkl.HIGHEST_PROTOCOL)
        if nn == 0:
    ...
    with open('./imgs/edges2shoes/test_images.pkl', \
              'wb') as f:
        pkl.dump([images, names], f, pkl.HIGHEST_PROTOCOL)
